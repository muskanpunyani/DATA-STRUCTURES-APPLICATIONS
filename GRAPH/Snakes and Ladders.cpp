...
public:
    int snakesAndLadders(vector<vector<int>>& board) {
        int n = board.size();
        vector<bool> seen(n*n+1,false);  // visited
		seen[1] = true;
        queue<pair<int,int>> q;  // 2nd parameter for distance
		q.push({1,0});
        while(!q.empty()){
            ...
            int s = p.first;
            int dist = p.second;
            if(s == n*n)   // if last val found return dist
                return dist;
            
            for(int i=1;s+i<=n*n && i<=6;i++){                
                getCoordinate(n,s+i,row,col);  // get coordinate for s+i
